            ...
            for i = #gameObjects, i, -1 do
                ...
                if gameObject ~= nil and gameObject.isDestroyed ~= true and gameObject ~= self.gameObject then
                    ...
                    elseif self.range <= 0 then
                        self.ray.position = gameObjectPosition   -- ray from the gameObject to the trigger
                        self.ray.direction = directionToTrigger:Normalized() 
                        local distance = nil
                        ...
                        if distance ~= nil and distance^2 >= distanceToTriggerSquared then
                            -- distance from the GO to the model or map hit location is superior to the distance to the trigger
                            -- that means the GO is inside of the model/map bloack
                            -- the ray goes throught the trigger origin before intersecting the map 
                            gameObjectIsInTrigger = true
                        end
                    end
