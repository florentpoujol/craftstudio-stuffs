--[[
The functions in this file extends some of Lua's built-in libraries.

Check each function declaration to learn more about what the function does, its argument(s) and returned value(s).
No functions in this file depends on CraftStudio, this is pure Lua.

math.isinteger( number )
math.lerp( a, b, factor[, easing] )
math.warpangle( angle )
math.round( value[, decimal] )
Credits for lerp(), warpangle() and round() goes to Élisée, from various projects
...
string.ucfirst( s )
string.lcfirst( s )
string.split( s, delimiter[, delimiterIsPattern=false] )
...
table.containsvalue( t, p_value[, ignoreCase] )
